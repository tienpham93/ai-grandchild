...
with col_left:
    ...
    if "confirm_delete_fam_id" in st.session_state:
        ...
        if c_del.button("Yes, Delete Entire Family", use_container_width=True):
    ...
    # Render Family grid headers
    grid_header = st.columns([1, 3, 2, 2])
    grid_header[0].markdown("**ID**")
    grid_header[1].markdown("**Family Directory Name**")
    grid_header[2].markdown("**View**")
    grid_header[3].markdown("**Remove**")
    st.write("---")
    ...
    if page_families:
        for fam in page_families:
            ...
            grid_row[0].write(str(fam["id"]))
            grid_row[1].write(fam["name"])
            ...
            if grid_row[3].button("Delete", key=f"d_fam_{fam['id']}"):
                st.session_state.confirm_delete_fam_id = fam["id"]
                st.rerun()
    else:
        st.info("No registered families found.")

# --- PANEL B: MEMBERS MANAGEMENT (ACTIVE FAMILIES SUB-GRID) ---
with col_right:
    if not st.session_state.selected_family_id:
        ...
        # Action button bar above sub-grid
        ...
        if c_act1.button("➕ Add Member", use_container_width=True):
            ...
            st.session_state.edit_member_target = None
        if c_act2.button("Close Family view"):
            st.session_state.selected_family_id = None
            st.session_state.selected_family_name = ""
            st.session_state.show_add_member_form = False
            st.session_state.edit_member_target = None
        ...
        if st.session_state.show_add_member_form:
            with st.container(border=True):
                ...
                if c_create.button("Create", disabled=not form_valid, use_container_width=True):
                    payload = {
        ...
        if "confirm_delete_mem_id" in st.session_state:
            ...
            if c_del_m.button("Yes, Revoke Access", use_container_width=True):
        ...
        m_header[0].markdown("**Name**")
        m_header[1].markdown("**Role**")
        m_header[2].markdown("**Telegram ID**")
        m_header[3].markdown("**Linked Bank Account**")
        m_header[4].markdown("**Edit**")
        m_header[5].markdown("**Delete**")
        st.write("---")
        ...
        if page_members:
            for mem in page_members:
                ...
                m_row[0].write(mem["name"])
                m_row[1].write(mem["member_role"])
                m_row[2].write(f"`{mem['chat_id']}`")
                
                b_acc = mem.get("bank_account")
                m_row[3].write(f"`{b_acc}`" if b_acc else "N/A")
                
                if m_row[4].button("Edit", key=f"e_mem_{mem['chat_id']}"):
                    st.session_state.edit_member_target = mem
                    st.session_state.show_add_member_form = False
                    st.rerun()
                    
                if m_row[5].button("Delete", key=f"del_mem_{mem['chat_id']}"):
